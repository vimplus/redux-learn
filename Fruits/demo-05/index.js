...
function fetchImportedApple(action) {
    fetching(2000, action)
}

// 采购进口鸡蛋3天（3s）
function fetchImportedEgg(action) {
    fetching(3000, action)
}
...
const reducer = combineReducers({ fruits: fruitsReducer, fresh: freshReducer });
// 创建店铺
...
store.dispatch(buyEgg(5));
store.dispatch(buyImportedEgg(5))
store.dispatch(buyImportedEgg(20))

store.dispatch(buyEgg(5));
store.dispatch(buyApple(10));
